...
class GCMAuthSubkeyRecoveryAttack(object):
    ...
    def _build_blockstrings(self, a1, c1, a2, c2):
        ...
        pad_c1 = '\x00'*(-len(c1) % 16)
        pad_c2 = '\x00'*(-len(c2) % 16)
    ...
    def _is_actual_key(self, k, data1, data2):\
        # Idea:
        #   * Build MAC polynomials qi out of ai and ci, i = 1, 2.
        #   * Convert tags t1, t2 to field elements.
        #   * If k is the real key, then q1(k) + t1 has to be the mask.
        #   * Thus, test whether q2(k) + s equals t2. If so, we found it!
        c1, a1, t1 = data1
        c2, a2, t2 = data2
        ...
        t2 = self._to_field_elem(t2)
        q2 = self._build_polynomial_2(a2, c2, str(), str())
    ...
    def recover_key(self, data1, data2):
        ...
        for candidate in candidates:
            if self._is_actual_key(candidate, data1, data2):
                return IntToBytes(candidate.n).value(16)